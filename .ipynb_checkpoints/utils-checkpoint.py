...
def get_chroma_collection(persist_dir: str = CHROMA_DIR, collection_name: str = COLLECTION_NAME):
    """
    Return (client, collection). This tries to use the same pattern as ingestion.
    Works with chromadb.PersistentClient or chromadb.Client depending on your package version.
    """
    try:
        # some chromadb versions provide PersistentClient (used in your ingest.py)
        client = chromadb.PersistentClient(path=persist_dir)
    except Exception:
        # fallback to chromadb.Client with Settings
        try:
...
def query_collection(question: str, k: int = 5) -> list:
    ...
    Each result is a dict: {'document': <text>, 'metadata': <meta dict>, 'distance': <float or None>}
    """
    client, coll = get_chroma_collection()
    embedder = get_embedder()
    q_emb = embedder.encode([question], convert_to_numpy=True).tolist()
    try:
        results = coll.query(query_embeddings=q_emb, n_results=k, include=["documents", "metadatas", "ids", "distances"])
        docs = results.get("documents", [])[0]
        metas = results.get("metadatas", [])[0]
        dists = results.get("distances", [])[0] if "distances" in results else [None] * len(docs)
    except Exception:
        # If the collection API differs, return empty list rather than crash.
        return []
    out = []
